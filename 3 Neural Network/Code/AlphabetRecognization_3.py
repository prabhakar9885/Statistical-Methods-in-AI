    ...
    def trainNN(self, imagesTrainSet, labelsTrainSet, etha):
        ...
        while j < 30:
            ...
            while i < trainingSetSize :
                ...
                J = z - lab;
                J = np.sum(0.5 * J * J);
                if J < 0.00001 and self.enableWeightDecay:
                    break;
...
def main(path):
    ...
    ar = AlphabetRecog(inputeUnits=28 * 28, hiddenUnits=100, outputUnits=10,
                       pathToDatasetFolder=path, enableWeightDecay=True);
    etha = 0.01
    i = 0;
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        path = "/home/prabhakar/IIIT-H_current/Sem 2/SMAI/Ass3_DataSet/"
    else:
        path = sys.argv[1];
    main(path)
