...
accuracyList = []
modelGivingHighAccuracy = 0
indexOfModelGivingHighAccuracy = -1
highestAccuracyTillNow = 0;
...
for i in range(10):
	...
	if highestAccuracyTillNow < accuracyList[i]:
		indexOfModelGivingHighAccuracy = i
		modelGivingHighAccuracy = ( meanForClass, SDForClass, probabilityForFeaturesGivenClass)

print "Avg. Accuracy: " + str( np.average(accuracyList) )
print "----------------------------------------\n"

p.dump( modelGivingHighAccuracy, open(filename+"-BEST-MODEL", "wb+") )
print "Model giving high accuracy is: " + str(i)
print "The model is picked(mean, SD, probability) in file: " + filename+"-BEST-MODEL"
print "\n"
